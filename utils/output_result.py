...
def output_results(csv_filename: str, game_results: List[Game]) -> None:
    ...
    for i in range(1, 10):  # Assuming maximum of 9 turns
        header.extend([f"turn_{i}_tiles_up", f"turn_{i}_dice_roll", f"turn_{i}_knocked_tiles"])
    ...
    with open(csv_filename, mode='w', newline='') as file:
        ...
        for result in game_results:
            row = [result.player_name, result.game_date, result.game_time, result.final_score]
